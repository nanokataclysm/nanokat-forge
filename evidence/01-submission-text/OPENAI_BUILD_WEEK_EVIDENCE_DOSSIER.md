...
- [x] Public demo works — https://nanokat-forge-z4l33yvnfq-uc.a.run.app (`/health` + plan + gated preview smoked)
- [ ] Narrated video is under three minutes (operator action)
- [ ] Agent disagreement is visible (**not in live slice** — Mission Society is design/spec; do not claim)
- [x] Granular approval is visible (approve required before preview; 409 without)
- [x] Validation and bounded preview path visible (preview validation object + isolated render; full multi-agent repair is not shipped)
- [x] Runtime provider is disclosed accurately (Qwen / Model Studio + Cloud Run)
